...
`Engine.ini` is not required for the camera fix. The installer can optionally turn chromatic aberration off there; see the README. Versions up to September 2026 also offered anti-blur TSR settings in the same managed block. They were dropped: `r.ScreenPercentage=100` outranks the game's own resolution scale (a value from `[SystemSettings]` beats scalability and game settings), `r.PostProcessAAQuality` no longer exists in these engine builds (UE 5.1 split it into `r.AntiAliasingMethod` and per-method quality variables; the string is absent from both executables), and the sharpening and mip bias were taste settings with shimmer and halo trade-offs. `restore` removes the old block by its markers, TSR lines included.
...
1. **`crates/installer`** - `lis-ultrawide-fix`, the installer: one binary for Windows and one static musl binary for Linux, the loader below embedded in each, no dependencies beyond the standard library and nothing downloaded, ever. Three independent parts, all on by default: the ultrawide camera (it writes the embedded loader next to the game executable as `winhttp.dll`, and under Proton registers it in the prefix's `user.reg` as a per-application `DllOverrides` entry), the full-width UI mod container (section 9c-3), and one `Engine.ini` tweak (chromatic aberration off) written as a removable managed block. `restore` undoes all of it, and a block an older version wrote with the anti-blur settings goes too, since the block is removed by its markers; `status` reports what is installed in lines a program can read; with no command it asks, at a terminal or through zenity, kdialog or yad, and without either it does nothing rather than install silently. Nothing it installs edits a shipped file, so there is nothing to back up; an executable that a version from before September 2026 patched in place is put back to stock from the `.original` backup those versions kept, once, and the backup is removed. It replaced a Python installer (`patcher.py`, in the repository's history up to 2026-09-02) whose on-disk results it reproduces exactly.
...
### 13j. The Recap Video (2026-09-05)

Reported: the pre-rendered "Previously On" video that opens a new game is stretched. It is `BP_VideoWindow` (`UI/BP/Window/`), a `D9Image` whose brush is `Movies/Global/BinkPlayer_Texture_Mat`, in a full-stretch slot of the window's `MainPanel` (anchors `(0,0)-(1,1)`, offsets `(0, 0, 0, -1)`). Stock, the window sits in the 3840x2160 `WindowParent` box and the image is 16:9; with the box widened to the design space (13i) the image follows and the picture stretches. `BP_MontageWindow`, which 13i edits, is the ending photo montage and unrelated.
...
### 13k. The Major-Choice Tint (2026-09-05)

Reported for both games: during a major choice the coloured zone over one side of the picture is stretched and no longer sits on what it is meant to highlight (a 32:9 screenshot showed it plainly; at 21:9 the stretch is 1.33x and easy to miss). It is not the choice UI: `BP_MajorChoiceWindow` is a `ScaleBox` (default `ScaleToFit`) over a 3840x2160 `SizeBox`, a self-contained 16:9 island that the widened `WindowParent` cannot touch, and `BP_MajorChoiceButton` is built from widget-relative slots only. The zone is `VFX/MajorChoice/PP_MajorChoice`, a post-process material with `LeftImage`, `RightImage` and `UpImage` texture parameters (`LeftSideBlend`, `RightSideBlend`, `UpSideBlend`, `OverallBlend`, `Blur Center` as scalars, no UV parameter) applied through `BP_MajorChoiceVFX`'s post-process component. The masks are sampled at screen position. Stock, the view is the 16:9 rect and a mask painted for it lines up; with cave A the view is the whole display, so a 16:9 mask is spread over the full width: the tint lands 1.33x wider at 21:9 and 2x at 32:9. Both games have the same setup (`PP_MajorChoice`, `PPI_MajorChoice`, `BP_MajorChoiceVFX`, `BP_MajorChoiceParameterActor`); Double Exposure also has `MF_MajorChoice_Mask`, and `Cine/PP/PP_MajorChoice01` is a separate distortion/vignette instance without masks.

Cooked materials carry no graph, so the material cannot be given a UV remap. The masks can be given one: every texture under `VFX/MajorChoice/Masks/` is re-fitted for the display and shipped in the mod container. Each mip is squeezed horizontally by `designW / 3840` into the centre of the same frame, and the columns beyond its edges repeat the edge column (`bc::refit`). Under the Hor+ camera the authored 16:9 picture is the centre band of the wide frame, so the squeezed mask lines up with it again; the repeated edge columns carry whatever the mask did at its edge out to the screen edge, so a tint that reached the 16:9 edge keeps going to the display edge and one that did not stays away from it. No seam, no letterbox in the effect. The masks are soft, so a linear resample loses nothing visible.

| Game | Masks | Formats | Sizes |
|---|---|---|---|
| Double Exposure | 29 | DXT1, DXT5 | 512x288 |
| Reunion | 19 | DXT1, DXT5, B8G8R8A8 | 512x288; 1127x635 (B8G8R8A8) |

The Double Exposure masks are per-scene (`EP1_S2A_MC_Intro_L/R`, `Ep2_S2B_MajorChoice_LeftMask`, ...) and hand-painted: a decoded one is a soft silhouette of the character on that side, the body in blue and the head in magenta, the channels being separate masks. Reunion's `Generic_MC_Mask_L/R` are half-screen fades, and its per-scene masks are uncompressed. Which mask a scene uses is keyed by the scene's data; the folder is the authored set, and the fix takes the folder, so a mask in it that is unlisted here is covered too.

**The cooked texture layout** (`crates/core/src/texture.rs`), the same in 5.2 and 5.5 for these textures: the unversioned properties, then a strip-flags word, the `PF_*` name (an `FName`, which is why it is in the name map), a 64-bit skip offset to the end, sixteen bytes, `SizeX`, `SizeY`, `PackedData`, the pixel format again as an `FString`, `FirstMipToSerialize`, `NumMips`, and for each mip one 32-bit word, the pixel data inline with no other header (the bulk-data meta is in the package's bulk data map), then `SizeX`, `SizeY`, `SizeZ`; then the virtual flag and the `None` name that closes the platform data list. The word ahead of each mip reads 0 in most textures, -1 in some and leftovers in one (`Ep2_S2X_MajorChoice_RightMask`), so nothing reads it; it is left as it is. The parser finds the format string and walks forward, checking each mip's trailer against the size it computed, which is what caught the word in the first place: with the data taken to start right after `NumMips`, every trailer read as `(0, 512, 288)`, and decoding under both alignments settled it (a clean mask against noise). Every mip is inline in these packages, so the re-encoded mip goes back in place at the same size and nothing else in the package or the container header changes.
